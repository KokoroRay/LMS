...
export default function Profile() {
  ...
  const getStudentCode = (u) =>
    u?.studentCode ??
    u?.profile?.studentCode ??
    u?.userProfile?.studentCode ??
    u?.__profile?.studentCode ??
    u?.profileDTO?.studentCode ??
    u?.student_code ??
    null;
  ...
  const init = useMemo(
    () => ({
      ...
      studentId: toStr(getStudentCode(user) ?? "—"),
    }),
    [user]
  );
    ...
    try {
      ...
      if (resUser) {
        form.setFieldsValue({
          ...
          studentId: toStr(getStudentCode(resUser) ?? init.studentId),
        });
              ...
              <Row gutter={16}>
                ...
                <Col xs={24} md={12}>
                  <Form.Item
                    name="studentId"
                    label="Mã sinh viên"
                    tooltip="Mã sinh viên là cố định và không thể thay đổi"
                  >
                    <Input
                      style={styles.input}
                      disabled
                      readOnly
                      value={init.studentId || "—"}
                    />
                  </Form.Item>
                </Col>
              </Row>
