...
const PostFormModal = ({ open, editingPost, onCancel, onComplete }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      if (editingPost) {
        ...
        form.setFieldsValue({
          authorId: editingPost.author?.id ?? 1,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let authorId = parseInt(values.authorId, 10);
      if (Number.isNaN(authorId) || authorId < 1) authorId = 1;
    ...
    >
      <Form form={form} layout="vertical" preserve={false}>
        <Form.Item
          name="authorId"
          label="Author ID"
          rules={[{ required: true, message: "Author ID là bắt buộc" }]}
          hidden
        >
